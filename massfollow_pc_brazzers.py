...
class Parse_follow:
    def __init__(self, LOGIN, PASS, link, quantity):
        ...
        self.root.destroy()
        self.browser = webdriver.Edge(self.webdriver_path)
        self.pers = []
        self.link = link
...
class Subscribe:
    ...
    def filter_person(self):
        ...
        no_person_en = '//*[@id="react-root"]/section/main/div/p'
        no_person_ru = '/html/body/div/div[1]/div/div/p'
        subs = '//*[@id="react-root"]/section/main/div/header/section/ul/li[3]/a/span'
        q_subs = 500
        count_pers = 0
        filtered = 0
        for person in self.persons:
            ...
            time.sleep(1)
            if self.find_element(no_person_en) == 1 or self.find_element(no_person_ru) == 1:
            ...
            if self.find_element(privat) == 1:
                ...
                self.del_subed()
                continue
            sub = self.browser.find_element_by_xpath(subs).text
            sub = re.sub(r'\s', '', sub)
            if int(sub) >= q_subs:
                print(str(count_pers) + ") More than {} follows".format(q_subs))
                self.del_subed()
                continue
            else:
                filtered += 1
    ...
    def subs(self):
        button_like = '/html/body/div[4]/div[2]/div/article/header/div[2]/div[1]/div[2]/button'
        button_sub = '/html/body/div[4]/div[2]/div/article/div[3]/section[1]/span[1]/button'
        time.sleep(2)
        ...
        time.sleep(2)
        if self.find_element(button_like) == 0:
            time.sleep(20)
            self.browser.get(self.current)
            time.sleep(2)
            if self.find_element(stor) == 1:
                self.browser.find_element_by_xpath(
                    '//*[@id="react-root"]/section/main/div/div[3]/article/div/div/div[1]/div[1]').click()
                ...
                self.browser.find_element_by_xpath(
                    '//*[@id="react-root"]/section/main/div/div[2]/article/div/div/div[1]/div[1]').click()
            time.sleep(2)
            # test time 20 work 10>?>?>?

        self.browser.find_element_by_xpath(button_like).click()
        self.browser.find_element_by_xpath(button_sub).click()
        time.sleep(1)
        self.counter += 1
